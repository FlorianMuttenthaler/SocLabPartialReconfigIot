...
#define TIMER_TCSR0	0x7E410000	// address of timer 0's control and status register
#define TIMER_TLR0	0x7E410004	// address of timer 0's load register
#define TIMER_TCR0	0x7E410008	// address of timer 0's counter register
...
#define DEVICE_NAME "blake2b"		// device name
...
ssize_t blake2b_write(struct file *flip, const char *buf, size_t length, loff_t *offset)
{
	printk("blake2b_write.\n");				// debug: procedure call message
/*
...
ssize_t blake2b_read(struct file *flip, char *buf, size_t length, loff_t *offset)
{
...
// open routine (called when a device opens /dev/blake2b)
static int blake2b_open(struct inode *inode, struct file *file)
{
	if (Device_Open)					// if the device is allready open,
		return -EBUSY;					// return with an error

	Device_Open++;						// 'open' device
	sprintf(msg,"You tried to open the blake2b module.\n");	// print open message
	msg_Ptr = msg;						
	try_module_get(THIS_MODULE);
	return 0;
}

// close routine (called whne a device closes /dev/blake2b)
static int blake2b_close(struct inode *inode, struct file *file)
{
...
struct file_operations blake2b_fops = {
	.read = blake2b_read,		// read()
	.write = blake2b_write,		// write()
	.open = blake2b_open,		// open()
	.release = blake2b_close,	// close()
};
...
{
	unsigned long temp;
	printk(KERN_ERR "Init blake2b module. \n");
...
/*
	...
	if (register_chrdev(SYSCALL_MAJOR, DEVICE_NAME, &blake2b_fops))
...
/*	iounmap(pTIMER_TCSR0);				// unregister timer hardware
	...
	printk(KERN_ERR "Exit blake2b Module. \n");	// print unload message
}
